...
if __name__ == "__main__":
    ...
    psp = pspnet(version="pascal")

    # Just need to do this one time
    caffemodel_dir_path = "./evaluation/model"
    # psp.load_pretrained_model(model_path=os.path.join(caffemodel_dir_path, 'pspnet101_VOC2012.caffemodel'))

    psp.load_state_dict(torch.load(os.path.join(caffemodel_dir_path, 'pspnet_101_pascalvoc.pth')))

    psp.float()
    psp.cuda(cd)
    psp.eval()

    dataset_root_dir = '/mnt/sqnap1/saugupt/public_datasets/PascalVoc2012/'
    dst = PascalVOCLoader(root=dataset_root_dir, is_transform=True, transform = None, augmentations=None, img_norm=False, split = 'trainval')
    img = m.imread("./messi.jpg")
    ...
    img = img.unsqueeze(0)
    # img = np.repeat(img[np.newaxis, :, :, :], 2, axis=0)
    print(img.shape)

    # print(psp(img.cuda())[0].shape)
    # print(psp(img.cuda())[1].shape)
    # print(isinstance(psp(img.cuda()), tuple))
    # print(type(psp(img.cuda())))

    # exit()
    
    output = psp.forward(img.cuda())
    print(output.shape)
    pp = output.data.max(1)[1]
    print(pp.size())

    # plt.imshow(pp.squeeze().cpu().detach().numpy())
    plt.show()

    out = psp.tile_predict(img)
    pred = np.argmax(out, axis=1)[0]
    decoded = dst.decode_segmap(pred)
    m.imsave("cropped.jpg", decoded)
    m.imsave('pred.jpg', pred)
    ...
    # torch.save(psp.state_dict(), os.path.join(caffemodel_dir_path, "pspnet_101_pascalvoc.pth"))
